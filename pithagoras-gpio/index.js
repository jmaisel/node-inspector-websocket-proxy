...
class GPIOPin {
    ...
    async input() {
        ...
        // Create GPIO input element in simulator
        await this.gpio.sendRequest('createGPIOInput', {
            bcmPin: this.pinNumber
        });
    ...
    async output() {
        ...
        // Create GPIO output element in simulator
        await this.gpio.sendRequest('createGPIOOutput', {
            bcmPin: this.pinNumber
        });
    ...
    async read() {
        if (this._direction === 'input') {
            ...
            throw new Error(`Pin ${this.pinNumber} not configured (call .input() or .output() first)`);
        }
    }
    ...
    async write(state) {
        if (this._direction !== 'input') {
            throw new Error(`Pin ${this.pinNumber} is not configured as input`);
        }
    ...
    onChange(callback) {
        if (this._direction !== 'output') {
            throw new Error(`Pin ${this.pinNumber} is not configured as output`);
        }
