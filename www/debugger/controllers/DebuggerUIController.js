...
export class DebuggerUIController extends BaseUIController {
     ...
     */
    updateControls() {
        $('#toolbar-pause-btn').prop('disabled', this.isPaused);
        $('#toolbar-resume-btn, #toolbar-step-over-btn, #toolbar-step-into-btn, #toolbar-step-out-btn').prop('disabled', !this.isPaused);
    }
    ...
    async connect() {
        try {
            const wsUrl = $('#toolbar-ws-url').val();
    ...
    disconnect() {
        ...
        if (this.debuggerUI?.consoleController) {
            this.debuggerUI.consoleController.clearConsole();
        }
    ...
    setupProtocolEvents() {
        ...
        this.client.debugger.on('Debugger.scriptParsed', function(event){
            ...
            // Add to file tree
            if (event.scriptId && event.url && that.debuggerUI?.fileTreeController) {
                that.debuggerUI.fileTreeController.addScriptToFileTree(event.scriptId, event.url);
            }
        ...
        this.client.debugger.on('Debugger.breakpointResolved', (event) => {
            ...
            if (this.debuggerUI?.breakpointController) {
                this.debuggerUI.breakpointController.handleBreakpointResolved(event);
            }
        });
    ...
    setupEventHandlers() {
        // Connection handlers
        $('#toolbar-connect-btn').click(() => this.connect());
        $('#toolbar-disconnect-btn').click(() => this.disconnect());

        // Control button handlers
        $('#toolbar-pause-btn').click(() => this.pause());
        $('#toolbar-resume-btn').click(() => this.resume());
        $('#toolbar-step-over-btn').click(() => this.stepOver());
        $('#toolbar-step-into-btn').click(() => this.stepInto());
        $('#toolbar-step-out-btn').click(() => this.stepOut());
    }
